...
# union of fact_internet_sales and fact_internet_sales_new tables
SELECT ProductKey,OrderDateKey,DueDateKey,ShipDateKey,CustomerKey,PromotionKey,CurrencyKey,SalesTerritoryKey,
...
# Explaination: it return union of two tables


#  Total Number of Orders
SELECT COUNT(Productkey) AS Total_Orders
FROM Fact_Internet_Sales;
# Explaination: Returns how many sales orders exist in the dataset.
...
SELECT p.EnglishProductName AS Product,ROUND(SUM(f.SalesAmount), 2) AS Total_Sales
...
SELECT pc.EnglishProductCategoryName AS Category,ROUND(SUM(f.SalesAmount), 2) AS Total_Sales
FROM Fact_Internet_Sales f
JOIN DimProduct p ON f.ProductKey = p.ProductKey
...
# Monthly Sales Trend
SELECT d.CalendarYear,d.EnglishMonthName,ROUND(SUM(f.SalesAmount), 2) AS Monthly_Sales
FROM Fact_Internet_Sales f
...
SELECT t.SalesTerritoryRegion,ROUND(SUM(f.SalesAmount), 2) AS Region_Revenue
FROM Fact_Internet_Sales f
...
SELECT t.SalesTerritoryRegion,COUNT(DISTINCT c.CustomerKey) AS Customer_Count
FROM DimCustomer c
...
(SELECT CustomerKey FROM Fact_Internet_Sales GROUP BY CustomerKey HAVING COUNT(DISTINCT SalesOrderNumber) > 1)
 AS multiple_orders;
# Explainatiom Number of customers who placed more than one order.

# Most Profitable Product
SELECT p.EnglishProductName AS Product,ROUND(SUM(f.SalesAmount - f.TotalProductCost), 2) AS Total_Profit
FROM Fact_Internet_Sales f
JOIN DimProduct p ON f.ProductKey = p.ProductKey
GROUP BY p.EnglishProductName
...
SELECT d.CalendarYear,ROUND(SUM(f.SalesAmount), 2) AS Total_Sales,
ROUND((SUM(f.SalesAmount) - LAG(SUM(f.SalesAmount)) OVER (ORDER BY d.CalendarYear)) / 
LAG(SUM(f.SalesAmount)) OVER (ORDER BY d.CalendarYear) * 100, 2) AS YoY_Growth_Percent
FROM Fact_Internet_Sales f
JOIN DimDate d ON f.OrderDateKey = d.FullDateAlternateKey
...
SELECT c.MaritalStatus,ROUND(SUM(f.SalesAmount), 2) AS Total_Sales
FROM DimCustomer c
JOIN Fact_Internet_Sales f ON c.CustomerKey = f.CustomerKey
...
# Sales by Country
SELECT t.SalesTerritoryCountry AS Country,
...
ORDER BY (SUM(TaxAmt) + SUM(Freight)) DESC LIMIT 5;
